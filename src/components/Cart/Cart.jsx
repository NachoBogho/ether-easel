...
const Cart = () => {
  ...
  return (
    <div className='cartDisplay'>
      {carrito.length > 0 ? <h2>Your Purchase</h2> : <></>}
      {carrito.map((prod) => (
      ...
      {carrito.length > 0 ? (
        <div className='displayFlexC'>
          ...
          <div className='displayButtonCart'>
            <button className='buttonCart' onClick={deleteCart}>
              Vaciar carrito
            </button>
            <Link to='/store'>
        ...
        <Link className='messageStore' to='/store'>
          Your cart is empty <br /> Go to the <span className='colorSpan'>STORE</span> to continue
        </Link>
      )}
    </div>
  );
